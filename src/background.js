...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === "APPLY_TAB_OUTPUT_DEVICE") {
    applyOutputDeviceToTargetTab(message.sinkId || "", message.tabId)
  ...
  if (message?.type === "GET_TAB_OUTPUT_DEVICES") {
    getOutputDevicesFromTargetTab(Boolean(message.requestPermission), message.tabId)
      .then((res) => sendResponse?.({ ok: true, ...res }))
      .catch((err) => sendResponse?.({ ok: false, error: err?.message || String(err) }));
    return true;
  }
...
async function getTargetTab(preferredTabId) {
  ...
  if (Number.isInteger(tabId) && tabId > 0) {
    try {
      ...
      // Fall back to the active tab if the opener tab no longer exists.
    }
  }
