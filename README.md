...
Chrome/Edge Manifest V3 extension for speech-to-text in a browser extension window, powered by the Web Speech API (`SpeechRecognition` / `webkitSpeechRecognition`).
...
Speech-to-text from an audio source, in an extension window, via the Web Speech API.
Completely free, no sign-up, no SMS!

The extension recognizes speech and shows the transcript in a separate extension window. It supports multiple sessions, live interim text, copy and `.txt` export.

## What it does

- Opens a separate extension window from the extension action button.
...
3. Select the VB-CABLE virtual speaker in the extension. Open the extension window, expand the "Speaker-Microphone" section, and select "CABLE Input (VB-Audio Virtual Cable)" from the list of output devices. The extension will route the current tab's audio to the virtual cable.
...
4. Click the extension action button to open the extension window.
